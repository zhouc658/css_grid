let capture;
// let modelURL = "https://teachablemachine.withgoogle.com/models/G3UIaNt9o/";
// let modelURL="https://teachablemachine.withgoogle.com/models/LyPBHTgoQ/"
// let modelURL="https://teachablemachine.withgoogle.com/models/yU18ui9d3/"
let modelURL="https://teachablemachine.withgoogle.com/models/KGuMxd-d4/"
//newest:https://teachablemachine.withgoogle.com/models/KGuMxd-d4/
let classifier;
let label = "...";

let sceneVideoStar, sceneVideoDog;
...
function setup() {
 ...
 // Step 1: Get available cameras and choose Logitech
...
function loadSceneVideos() {
 ...
 // Masking
 videoBuffer = createGraphics(680, 680);
...
function draw() {
 ...
 if (videoPlayingDog) currentVideo = sceneVideoDog;


 if (!videoPlayingStar && !videoPlayingDog) {
   fill(255);
   noStroke();
   ellipse(width / 2, height / 2, 680, 680);
 } else if (currentVideo) {
   videoBuffer.image(currentVideo, 0, 0, 680, 680);
   maskGraphics.clear();
   maskGraphics.ellipse(340, 340, 680, 680);
   let masked = videoBuffer.get();
...
function getResults(results, error) {
 ...
 if (results && results[0]) {
   ...
   if (topResult.confidence < 0.7) {
     currentLabel = "none";
   } else if (topResult.label === "bun" || topResult.label === "dog") {
